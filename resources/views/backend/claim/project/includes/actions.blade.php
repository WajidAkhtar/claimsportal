...
@if($project->created_by == auth()->user()->id || auth()->user()->hasRole('Super User') || auth()->user()->hasRole('Administrator'))
	<x-utils.edit-button :href="route('admin.claim.project.edit', $project)" />
	@if(auth()->user()->hasRole('Administrator') || auth()->user()->hasRole('Super User') || $project->created_by == auth()->user()->id)
